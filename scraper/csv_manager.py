...
from scraper.utils import american_profit_multiple


SCRAPER_DIR = Path(__file__).resolve().parent
RECENT_CSV_PATH = SCRAPER_DIR / "recent.csv"
MISSING_REPORT_PATH = SCRAPER_DIR / "missing_data_report.csv"
MISSING_SUMMARY_PATH = SCRAPER_DIR / "missing_columns_summary.csv"
DEFAULT_ODDS_REPORT_PATH = SCRAPER_DIR / "default_odds_report.csv"
...
def save_recent_dataframe(df: pd.DataFrame) -> Path:
    df[RECENT_COLUMNS].to_csv(RECENT_CSV_PATH, index=False)
...
def save_missing_reports(df: pd.DataFrame) -> tuple[Path, Path]:
    ...
    return MISSING_REPORT_PATH, MISSING_SUMMARY_PATH


def save_default_odds_report(df: pd.DataFrame) -> Path:
    default_odds = df.loc[
        df["odds_source"] == "default_missing",
        ["fight_date", "event_name", "red_fighter", "blue_fighter", "odds_source", "odds_note"],
    ].copy()
    default_odds.to_csv(DEFAULT_ODDS_REPORT_PATH, index=False)
    return DEFAULT_ODDS_REPORT_PATH
