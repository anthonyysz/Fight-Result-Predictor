...
from scraper.csv_manager import (
    RECENT_CSV_PATH,
    create_empty_recent_dataframe,
    finalize_recent_dataframe,
    save_default_odds_report,
    save_missing_reports,
    save_recent_dataframe,
)
from scraper.odds_scraper import apply_odds
from scraper.rankings_scraper import apply_rankings
from scraper.ufcstats_scraper import apply_ufcstats_data, initialize_recent_rows
from scraper.utils import parse_us_date
...
SQL_ENV_PATH = REPO_DIR / "sqlData" / ".env"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build scraper/recent.csv from 2024-12-14 to present.")
    parser.add_argument(
        "--start-date",
...
def main() -> None:
    ...
    default_odds_report_path = save_default_odds_report(df_internal)
    ...
    print(f"recent_csv: {RECENT_CSV_PATH}")
    print(f"missing_report: {missing_report_path}")
    print(f"missing_summary: {missing_summary_path}")
    print(f"default_odds_report: {default_odds_report_path}")
