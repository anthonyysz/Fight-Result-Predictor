...
from scraper.aliases import AliasRegistry, load_alias_registry
from scraper.utils import clean_text, normalize_name
...
def apply_odds(df: pd.DataFrame, alias_csv_path) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if event is None:
            row_dict["red_odds"] = DEFAULT_MISSING_ODDS
            row_dict["blue_odds"] = DEFAULT_MISSING_ODDS
            row_dict["odds_source"] = "default_missing"
            row_dict["odds_note"] = "no FightOdds event found for event date"
...
def match_fight(event_offer_table: dict, red_fighter: str, blue_fighter: str, aliases: AliasRegistry) -> dict[str, object]:
    ...
    for edge in event_offer_table["fightOffers"]["edges"]:
        ...
        if preferred_offer is None:
            candidate = {
                "red_odds": DEFAULT_MISSING_ODDS,
                "blue_odds": DEFAULT_MISSING_ODDS,
                "odds_source": "default_missing",
                "odds_note": "matched fight but no FanDuel/BetUS/MyBookie odds found; defaulted to -10000",
            }
    ...
    return {
        ...
        "blue_odds": DEFAULT_MISSING_ODDS,
        "odds_source": "default_missing",
        "odds_note": "no FightOdds bout match found; defaulted to -10000",
    }
