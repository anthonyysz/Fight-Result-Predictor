...
def fetch_cumulative_profit_frame(conn) -> pd.DataFrame:
    query = """
        SELECT
            fight_date,
            SUM(model_return - 1.0)::float AS event_units,
            COUNT(*) AS bet_count
        FROM public.historical_predictions
        WHERE model_pick != 'Pass'
          AND model_return IS NOT NULL
        GROUP BY fight_date
        ORDER BY fight_date
    """
    ...
    frame["cumulative_bets"] = frame["bet_count"].cumsum()
    frame["cumulative_roi"] = frame["cumulative_units"] / frame["cumulative_bets"]
    return frame
...
def render_average_return_chart(conn: Any) -> bytes:
    ...
    ax.set_title("Cumulative Flat-Bet ROI by Fight Date", fontsize=18, fontweight="bold", pad=16)
    ax.set_xlabel("Fight Date", labelpad=10)
    ax.set_ylabel("Profit per Unit Risked", labelpad=10)
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %d"))
    max_roi = frame["cumulative_roi"].max()
    upper_limit = max(1.0, math.ceil((max_roi + 0.05) * 2) / 2)
    ax.set_ylim(-1.0, upper_limit)
    ax.yaxis.set_major_locator(mtick.MultipleLocator(0.5))
    ax.yaxis.set_major_formatter(mtick.FuncFormatter(lambda value, _: f"{value:+.1f}x"))
